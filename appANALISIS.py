...
def inicializar_estado():
    """Inicializa variables de estado y datos de ejemplo para el historial."""
    ...
    if 'indice_simulacion' not in st.session_state:
        st.session_state['indice_simulacion'] = 0 
...
def renderizar_interfaz_seguimiento():
    ...
    elif i < NUM_PUNTOS_SIMULACION and df_actividad['Ritmo_Inst_Seg_Km'].iloc[i-1] >= 900:
        estado_pausa = "PAUSA AUTOMÁTICA"

    # --- 3.1. Dibujar Métricas ---
    st.markdown(f"**Estado:** {estado_pausa} | Punto de Datos: {i} de {NUM_PUNTOS_SIMULACION}")
    ...
    # --- 3.2. Dibujar Mapa (Actualización Condicional) ---
    st.subheader("Ruta en Tiempo Real con Intensidad")
    
    # Solo dibujamos el mapa si hemos avanzado y es un múltiplo de MAPA_ACTUALIZAR_CADA
    if i > 0 and (i % MAPA_ACTUALIZAR_CADA == 0 or i == NUM_PUNTOS_SIMULACION): 
    ...
    # --- 3.3. Controles de Avance y Finalización (Keys únicos corregidos) ---
    
    if i < NUM_PUNTOS_SIMULACION:
        col_pausa, col_finalizar, col_avance = st.columns([1, 1, 1])
        
        # 🛑 CORRECCIÓN: Usando keys únicos
        if col_pausa.button("🔴 PAUSA", key="sim_pausa_manual"):
             st.warning("Pausa manual activada. Haz clic en 'AVANZAR SIMULACIÓN' para reanudar.")
        
        if col_finalizar.button("✅ FINALIZAR Y GUARDAR", type="primary", key="sim_finalizar_manual"):
            finalizar_actividad_manual(df_actividad)
        
        if col_avance.button(f"▶️ AVANZAR SIMULACIÓN ({i+1}/{NUM_PUNTOS_SIMULACION})", key="sim_avanzar_simulacion"):
            avanzar_simulacion()
            
    else:
        # Si la simulación ha llegado al final
        if not st.session_state['actividad_finalizada']:
            finalizar_actividad_manual(df_actividad)
            
        st.success("Actividad Finalizada. ¡Revisa el Análisis y Estadísticas!")
        st.rerun() # Asegura la transición a la pantalla de análisis
...
def renderizar_pantalla_analisis():
    ...
    with tab_entrenamiento:
        ...
        with st.expander("➕ Crear Sesión de Intervalos (Premium)", expanded=False):
            num_repeticiones = st.slider("Número de Repeticiones", 1, 20, 8, key="rep_slider")
            tiempo_rapido = st.number_input("Tiempo de Tramos Rápidos (segundos)", 30, 300, 60, key="rapido_input")
            tiempo_recuperacion = st.number_input("Tiempo de Recuperación (segundos)", 30, 300, 90, key="recup_input")

            if st.button("Guardar Entrenamiento de Intervalos", key="guardar_entrenamiento"):
    ...
    st.markdown("---")
    if st.button("⬅️ VOLVER A INICIO", key="volver_a_inicio"):
        st.session_state['actividad_finalizada'] = False
        st.rerun()
...
if st.session_state['actividad_finalizada']:
    ...
    st.write(" ")
    if st.button("▶️ INICIAR ACTIVIDAD", type="primary", use_container_width=True, key="iniciar_app"):
